import { NavLink, useLocation } from 'react-router-dom';

import BrandLogo from '../brand/BrandLogo';

const servicePaths = [
  '/services',
  '/creation-sites-web',
  '/logiciels-sur-mesure',
  '/support-it-securite',
  '/formation'
];

function isServicesRoute(pathname) {
  return servicePaths.includes(pathname);
}

function ThemeIcon({ isDark }) {
  if (isDark) {
    return (
      <svg className="sun-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        <circle cx="12" cy="12" r="5" />
        <line x1="12" y1="1" x2="12" y2="3" />
        <line x1="12" y1="21" x2="12" y2="23" />
        <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
        <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
        <line x1="1" y1="12" x2="3" y2="12" />
        <line x1="21" y1="12" x2="23" y2="12" />
        <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
        <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
      </svg>
    );
  }

  return (
    <svg className="moon-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
    </svg>
  );
}

export default function Header({
  navigation,
  onToggleMenu,
  onToggleTheme,
  isDark,
  isMenuOpen,
  scrolled
}) {
  const { pathname } = useLocation();

  return (
    <nav className={`nav ${scrolled ? 'scrolled' : ''}`} id="mainNav">
      <BrandLogo size="header" />

      <ul className="nav-links">
        {navigation.map((item) => {
          if (item.label === 'Services') {
            return (
              <li key={item.path}>
                <NavLink
                  to={item.path}
                  className={isServicesRoute(pathname) ? 'active-link' : ''}
                  aria-current={isServicesRoute(pathname) ? 'page' : undefined}
                >
                  {item.label}
                </NavLink>
              </li>
            );
          }

          return (
            <li key={item.path}>
              <NavLink
                to={item.path}
                className={({ isActive }) =>
                  [isActive ? 'active-link' : '', item.cta ? 'nav-cta' : ''].filter(Boolean).join(' ')
                }
              >
                {item.label}
              </NavLink>
            </li>
          );
        })}
      </ul>

      <button
        className="theme-toggle"
        type="button"
        aria-label="Changer de theme"
        title="Changer de theme"
        onClick={onToggleTheme}
      >
        <div className="theme-toggle-icon">
          <ThemeIcon isDark={isDark} />
        </div>
      </button>

      <button
        className={`nav-burger ${isMenuOpen ? 'open' : ''}`}
        id="burger"
        type="button"
        aria-label="Menu de navigation"
        aria-controls="mobileMenu"
        aria-expanded={isMenuOpen}
        onClick={onToggleMenu}
      >
        <span />
        <span />
        <span />
      </button>
    </nav>
  );
}
